...
class CustomSerializer extends Serializer {
    ...
    foreach ($this->view->result as $row_index => $row) {
      ...
      if ($user->id() != 0) {
        ...
        $query
          ...
          ->condition('entity_id', $row->_entity->get('nid')->value)
          ->condition('type', 'like')
          ->fields('votes', ['id']);
        $liked_id = $query->execute()->fetchField(0);
        $query1 = $database->select('votingapi_vote', 'votes');
        $query1
          ->condition('user_id', $user->id())
          ->condition('entity_id', $row->_entity->get('nid')->value)
          ->condition('type', 'wishlist')
          ->fields('votes', ['id']);
        $wish_id = $query1->execute()->fetchField(0);
      }

      $object = [
        "content" => $this->view->rowPlugin->render($row),
        "relation" => $row->_relationship_entities,
        "like" => $liked_id == '' ? [] : [
          "nid" => [["value" => $liked_id]],
        ],
        "wish" => $wish_id == '' ? [] : [
      ...
      $node = $node_storage->load($row->_entity->get('nid')->value);
      ...
      foreach ($taxonomy as $field) {
        $object['taxonomy'][$field] = $this->getTaxonomyDetail($row, $field);
      }
      $rows[] = $object;
    }
  ...
  public function getTaxonomyDetail($row, $field) {
    $result = [];
    if ($row->_entity->hasField($field)) {
      $package = $row->_entity->get($field)->getString();
